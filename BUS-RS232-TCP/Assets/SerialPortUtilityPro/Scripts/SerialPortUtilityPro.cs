...
namespace SerialPortUtility
{
	...
	{
		//Version Infomation
		public const string VersionString = "2.72";
		...
		void Start()
		{
			...
			if (DebugConsoleMonitor)    // && Debug.isDebugBuild
			{
				...
				{
					if (transform.name == "[Script] (COM3)SerialPortUtilityProObject")
					{
                        GameObject obj = Resources.Load<GameObject>($"Prefabs/SPUPDebugConsole_COM3");
                        DebugConsoleObject = Instantiate(obj);
                        DebugConsoleObject.transform.SetParent(this.transform); //parent set
                    }
					else if (transform.name == "[Script] (COM5)SerialPortUtilityProObject")
					{
                        GameObject obj = Resources.Load<GameObject>($"Prefabs/SPUPDebugConsole_COM5");
                        DebugConsoleObject = Instantiate(obj);
                        DebugConsoleObject.transform.SetParent(this.transform); //parent set
                    }
				}
